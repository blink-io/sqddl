...
// ModelsCmd implements the `sqddl models` subcommand.
...
func ModelsCommand(args ...string) (*ModelsCmd, error) {
	var cmd ModelsCmd
  ...
  sqddl models -db <DATABASE_URL> [FLAGS]
  sqddl models -db 'postgres://user:pass@localhost:5432/sakila'
  sqddl models -db 'postgres://user:pass@localhost:5432/sakila' -pkg tables -file models/models.go
  sqddl models -db 'postgres://user:pass@localhost:5432/sakila' -schemas schema1,schema2,schema3 -exclude-tables table1,table2,table3
Flags:
`)
...
func (cmd *ModelsCmd) Run() error {
	...
	if cmd.SchemaQualifiedStructs {
		for i := range modelStructs.Models {
			modelStruct := &modelStructs.Models[i]
